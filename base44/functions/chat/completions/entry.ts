...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Strip search suffix for model resolution
    ...
    if (webSearchEnabled) {
      // Only gemini supports internet search
      if (!internalModel.startsWith('gemini')) {
    ...
    if (!skipCache) {
      const cacheInput = JSON.stringify({
        ...
        reasoning: thinkingEnabled ? (effortRaw || 'high') : null,
      ...
      if (cached && cached.length > 0) {
        ...
        if (!entry.expires_at || new Date(entry.expires_at) > now) {
          ...
          if (stream) {
            // Stream cached response quickly (~500ms total)
            ...
            const readable = new ReadableStream({
              async start(controller) {
                for (let i = 0; i < chars.length; i += chunkSize) {
                  ...
                  const delta = { id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [{ index: 0, delta: { content: chunk }, finish_reason: null }], cached: true };
                  controller.enqueue(encoder.encode(`data: ${JSON.stringify(delta)}\n\n`));
                  await new Promise(r => setTimeout(r, 25));
                }
                const finalChunk = { id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [{ index: 0, delta: {}, finish_reason: 'stop' }], cached: true };
                controller.enqueue(encoder.encode(`data: ${JSON.stringify(finalChunk)}\n\n`));
                if (stream_options?.include_usage) {
                  const usageChunk = { id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [], usage: { prompt_tokens: entry.prompt_tokens || 0, completion_tokens: entry.completion_tokens || 0, total_tokens: entry.total_tokens || 0 } };
    ...
    if (!skipCache && cacheKey) {
      ...
      // Prefix backfill: cache (prefix → assistant reply) for every assistant turn in history.
    ...
    } catch (_) { /* ignore stats write errors */ }
    ...
    if (stream) {
      ...
      // Adaptive chunk size: aim for ~80 chunks
      const chunkSize = Math.max(3, Math.ceil(totalChars / 80));
      const encoder = new TextEncoder();

      const readable = new ReadableStream({
        async start(controller) {
          // First chunk: role
          const roleChunk = { id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [{ index: 0, delta: { role: 'assistant', content: '' }, finish_reason: null }] };
          controller.enqueue(encoder.encode(`data: ${JSON.stringify(roleChunk)}\n\n`));

          for (let i = 0; i < chars.length; i += chunkSize) {
            const chunk = chars.slice(i, i + chunkSize).join('');
            const delta = { id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [{ index: 0, delta: { content: chunk }, finish_reason: null }] };
            controller.enqueue(encoder.encode(`data: ${JSON.stringify(delta)}\n\n`));
            await new Promise(r => setTimeout(r, 10));
          }

          const finalChunk = { id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [{ index: 0, delta: {}, finish_reason: 'stop' }] };
          controller.enqueue(encoder.encode(`data: ${JSON.stringify(finalChunk)}\n\n`));

          if (stream_options?.include_usage) {
            const usageChunk = {
              id: completionId, object: 'chat.completion.chunk', created: Math.floor(Date.now() / 1000), model: requestedModel, choices: [],
              usage: {
                prompt_tokens: promptTokens,
                completion_tokens: completionTokens,
                total_tokens: totalTokens,
